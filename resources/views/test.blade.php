...
class NewsController extends Controller
{
    ...
    public function addNews(Request $request) {
        ...
        if($image->getSize() > 500 * 1024) {
            ...
            $resizedImage->save(public_path('../../public_html/images/'. $newImageName));
        }

        else {
            $newImageName = time(). '.' . $request->image->extension();
            $request->image->move(public_path('../../public_html/images'), $newImageName);
        }
    ...
    public function editNewsPost(Request $request, $id) {
        ...
        if(empty($request->image)) {
            ...
            File::delete(public_path('../../public_html/images/'.$request->initialImage));
    ...
    public function deleteNews($id) {
        ...
        if($news) {
            $image = $news->image;
            $news->delete();
            File::delete(public_path('../../public_html/images/'.$image));
